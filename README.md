...
### v0.5.3
- **Fix:** the per-source app filter had silently stopped applying, so Apple TV **video** followed occupancy between rooms just like music. The stored source had `app_ids: ["com.apple.TVMusic"]` but `app_filter_entity: ""`, and `resolve_routing()` required *both* to be set before checking anything — a blank filter entity meant "no filter", allowing every app, with no log line to say so. `resolve_routing()` now treats a populated `app_ids` as authoritative: if the filter entity is blank it falls back to the source's own `watcher_entity` and logs a warning once, rather than routing everything.
- **Fix:** the options flow was what blanked the field. When the pre-existing `entry.data` source dict lacked `app_filter_entity` (it predates v0.3.10), the schema offered the entity field with no default, and on save `user_input.get(...) or ""` persisted an empty string. Both source steps now default the filter entity to the source's watcher whenever an app list is configured, so it can no longer be saved blank.
- **Fix:** `app_ids`, `bypass_app_ids`, and `party_mode_trigger_apps` select-selectors were built only from the target entity's *current* app, so re-saving options while the Apple TV happened to be on a different app could drop the stored allow-list. Their option lists now always include the already-stored values.
- **Added:** config-entry migration (`minor_version` 2) that backfills a blank `app_filter_entity` from `watcher_entity` on any source that has an `app_ids` allow-list, repairing entries already affected.
- **Added:** labels for `app_filter_entity`, `app_ids`, and `bypass_app_ids` in `strings.json`/`translations/en.json` — these fields previously rendered unlabeled, which is how the filter entity got cleared in the first place. Hints now state that matching is exact and that an empty app list means "follow on any app, including video".
- **Added:** the system Status sensor exposes an `app_filters` attribute (per source: `app_ids`, effective `filter_entity`, `filtered`), so a disabled filter is visible in the UI instead of only in `.storage/core.config_entries`.
- **Fix:** per-source Follow Me switches were built from `entry.data["sources"]` (data-first), the opposite precedence from the coordinator and options flow. After a source was renamed in options, the switch was keyed to the old display name and governed nothing — `resolve_routing()` fell back to its `True` default, so that source's Follow Me could not be turned off. Now built from `coordinator.get_sources()`. **Note:** this changes the affected switch's entity id to match the current display name.
- **Docs:** corrected `_app_matches`' docstring, which claimed case-insensitive *substring* matching; it has always been exact equality against `app_id`/`app_name`. `Music` does not match `Apple Music` — use the full `app_id`.
